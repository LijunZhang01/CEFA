...
class HICOEvaluatorKO():
        ...
        if not self.eval_location:
            ...
            def fetch_follower_likes(s_e):
                ...
                for index, img_preds in enumerate(cur_preds):
                    img_preds = {k: v.to('cpu').numpy() for k, v in img_preds.items()}
                    # bboxes = [{'bbox': list(bbox)} for bbox in img_preds['boxes']]
                    bboxes = [{'bbox': list(bbox),"category_id":lab} for bbox,lab in zip(img_preds['boxes'],img_preds['labels'])]
                    obj_scores = img_preds['obj_scores'] * img_preds['obj_scores']
                    hoi_scores = img_preds['hoi_scores'] + obj_scores[:, self.hoi_obj_list]
        ...
        def fetch_follower_likes(s_e):
            ...
            for img_preds, img_gts in zip(cur_preds, cur_gts):
                ...
                for item in pred_hois:
                    # if self.hoi_obj_list[item['category_id']] in cat_ids:
    ...
    def compute_map_ko(self):
        ...
        if self.zero_shot_type == "default":
            print('mAP full ko: {} mAP rare ko: {}  mAP non-rare ko: {}  mean max recall: {}'.format(m_ap, m_ap_rare, m_ap_non_rare,
                                                                                    m_max_recall))
            ...
            print('mAP full ko: {} mAP unseen-obj ko: {}  mAP seen-obj ko: {}  mean max recall: {}'.format(m_ap, m_ap_rare, m_ap_non_rare,
                                                                                    m_max_recall))
            return_dict = {'mAP': m_ap, 'mAP unseen-obj': m_ap_rare, 'mAP seen-obj': m_ap_non_rare, 'mean max recall': m_max_recall}

        else:
            print('mAP full ko: {} mAP unseen ko: {}  mAP seen ko: {}  mean max recall ko: {}'.format(m_ap, m_ap_rare, m_ap_non_rare,
                                                                                    m_max_recall))
