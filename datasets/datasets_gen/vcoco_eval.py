...
class VCOCOEvaluator():
    ...
    def __init__(self, preds, gts, correct_mat, use_nms_filter=False):
        ...
        self.rare_indices = [24,25,26,27]
        dd=range(29)
        self.norare_indices=[x for x in dd if x not in self.rare_indices]
    ...
    def compute_map(self):
        ...
        rareap_thesis = np.mean([ap[category_id] for category_id in self.rare_indices])
        norareap_thesis = np.mean([ap[category_id] for category_id in self.norare_indices])

        print('------------------------------------------------------------')
        print('mAP all: {:.4f} mAP thesis: {:.4f} mAP rare_thesis: {:.4f} mAP norare_thesis: {:.4f}'.format(m_ap_all, m_ap_thesis,rareap_thesis,norareap_thesis))
        print('------------------------------------------------------------')
        aps.update({'mAP_all': m_ap_all, 'mAP_thesis': m_ap_thesis, 'mAP_rare_thesis': rareap_thesis, 'mAP_norare_thesis': norareap_thesis})
